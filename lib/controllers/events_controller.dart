...
class EventController extends GetxController {
  ...
  final Rx<Uint8List?> _pickedImageWeb = Rx<Uint8List?>(null);
  final Rx<String?> _pickedImageUrl = Rx<String?>(null);

  File? get posterPhoto => _pickedImage.value;
  Uint8List? get posterPhotoWeb => _pickedImageWeb.value;
  String? get posterImageUrl => _pickedImageUrl.value;
  ...
  /// Upload image to ImgBB (free, no Firebase Storage bucket needed).
  Future<String> _uploadImage(dynamic imageSource) async {
    try {
      Uint8List imageBytes;
      if (kIsWeb) {
        imageBytes = imageSource as Uint8List;
      } else {
        imageBytes = await (imageSource as File).readAsBytes();
      }
      return await ImgBbService.uploadImage(imageBytes);
    } catch (e) {
  ...
  void pickImage() async {
    ...
    if (pickedImage != null) {
      ...
      if (kIsWeb) {
        ...
        _pickedImageWeb.value = imageBytes;
        _pickedImageUrl.value = pickedImage.path;
      } else {
  ...
  Future<void> addEvent(
    ...
    try {
      if (addFormKey.currentState!.validate()) {
        ...
        String posterUrl;
        if (kIsWeb) {
          if (_pickedImageWeb.value == null) {
            Get.snackbar(
              'Error!',
              'Please select an event poster image.',
            );
            toggleLoading();
            return;
          }
          posterUrl = await _uploadImage(_pickedImageWeb.value!);
        } else {
          ...
          posterUrl = await _uploadImage(_pickedImage.value!);
        }
  ...
  Future<void> generateAndSaveQrCode(User user, String eventId) async {
    try {
      ...
      // Upload QR image to ImgBB (no Firebase Storage bucket needed)
      final downloadUrl = await ImgBbService.uploadImage(pngBytesList);
  ...
  void removeParticipantFromEvent(String participantId, String eventId) async {
    try {
      ...
      // Note: QR images are on ImgBB - no need to delete from storage
  ...
  void resetFields() {
    ...
    _pickedImageWeb.value = null;
    _pickedImageUrl.value = null;
  }
}
